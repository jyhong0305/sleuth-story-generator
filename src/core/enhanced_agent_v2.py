...
class PaperBasedAgent:
    ...
    def _get_relevant_memories(self, query: str, k: int = 10) -> List[str]:
        ...
        nodes = self.memory.retrieve(query, current_time, top_k=k)
    ...
    def autonomous_act(self, current_time: datetime, all_agents: List['PaperBasedAgent']):
        """자율 행동"""
        ...
        # [논문] 기억 저장
        self.memory.add_memory(observation, current_time, node_type="observation")
        ...
        decision = self.planning.decide_action(
            observation, current_plan, people_nearby, current_time
        )
        ...
        if result:
            self.memory.add_memory(
                f"{current_time.strftime('%H:%M')} - {result}",
                current_time,
    ...
    # _execute_move, _execute_chat 등은 기존과 동일
    # ... (이전 코드 복사)
